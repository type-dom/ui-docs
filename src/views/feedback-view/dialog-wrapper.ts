...
import { DialogCenteredContentExample } from '../../examples/feedback/dialog/centered-content';
import { DialogAlignCenterExample } from '../../examples/feedback/dialog/align-center';
import { DialogDestroyOnCloseExample } from '../../examples/feedback/dialog/destroy-on-close';
import { DialogDraggableExample } from '../../examples/feedback/dialog/draggable';
...
export class DialogWrapper extends TypeDiv {
  ...
  mounted() {
    ...
    fetch('./examples/feedback/dialog/nested.ts')
      .then(res => res.text())
      .then(text => {
        this.sourceData.nested.setValue(text);
      })
      .catch(err => console.error('fetch error:', err));
  ...
  createCustomizationContent() {
    this.addChildren(
      ...
      new Example({
        ...
        sourceWrapper: this.sourceData.customizationContent
      })
    );
  }
  ...
  createCustomizationHeader() {
    this.addChildren(
      ...
      new Example({
        ...
        sourceWrapper: this.sourceData.customizationHeader
      })
    );
  }
  ...
  createCenteredContent() {
    this.addChildren(
      ...
      new Example({
        showcase: [new DialogCenteredContentExample()],
